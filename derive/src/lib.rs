...
use quote::quote;
use syn::{Attribute, DeriveInput, Expr, Field, GenericParam, Ident, Type, TypePath, parse_str, punctuated::Punctuated, token::Comma, ExprLit, Lit};
        ...
        let arms = data.variants.iter().map(|va|
            {
                ...
                if let Some((_, Expr::Lit(ExprLit{lit: Lit::Int(v), ..}))) = &va.discriminant {
